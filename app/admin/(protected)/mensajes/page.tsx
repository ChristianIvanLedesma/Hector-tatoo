      ...
      p-4 shadow-lg transition hover:scale-[1.02] rounded-br-xs"
        >
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-white font-semibold text-lg">{m.nombre} {m.apellido}</h3>
            <span className="text-xs text-red-300">#{m.id}</span>
          </div>

          <div className="space-y-1 text-sm text-gray-200">
            <p>📧 <span className="font-medium">{m.correo}</span></p>
            <p>📞 <span className="font-medium">{m.telefono}</span></p>
          </div>
